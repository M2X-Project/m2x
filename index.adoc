...
image::lhs_self_managed_area.svg[title="Exemplary use case of a load handling station with two load handling points", 640,480,align="center", id=load_handling_station_view,align="center"]

image::lhs_with_traffic_management.svg[title="Exemplary use case of a load handling station with two load handling points and a shared traffic management area", 640,480,align="center", id=lhs_with_traffic_management,align="center"]
...
The communication with a load handling station is done via the following topics:
...
|Request | /lhs/v1.0.0/lhs_example/request* | Used for communication of requests for different load transfer stages: requestAccess, requestLoadTransfer, requestExit, revokeAccess, requestFactsheet
...
|*loadHandlingPointStates* *[loadHandlingPointState]* {|object|Describes upcoming and current requests.
...
|{nbsp}{nbsp}{nbsp}{nbsp}theta|number|Angle for handover position.
|}||
|*tolerance* {|object|Acceptable tolerance for the load transfer.
|{nbsp}{nbsp}{nbsp}{nbsp}x|number|X-position in factory-coordinate system.
|{nbsp}{nbsp}{nbsp}{nbsp}y|number|Y-position in factory-coordinate system.
|{nbsp}{nbsp}{nbsp}{nbsp}theta|number|Deviating tolerance for handover position.
|{nbsp}{nbsp}{nbsp}{nbsp}}||
|}||
...
|{nbsp}{nbsp}{nbsp}{nbsp}restrictedVehicles [restrictedVehicle]|array|List of all MRs that are not allowed to interact at the load handling point. If this parameter does not exist or is empty, the load handling point is valid for all MRs on this station.
|{nbsp}{nbsp}{nbsp}{nbsp}restrictedLoads[restrictedLoad]|array|List of all loads that are not allowed to interact at the load handling point. If this parameter does not exist or is empty, the load handling point is valid for all MRs on this station.
|}||
|===
...
=== Load handling interface
The JSON schema link:assets/json_schemas/load_handling_unit.schema[load_handling_unit] is described below for seamless implementation. 
